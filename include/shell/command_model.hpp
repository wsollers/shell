...
using EnvMap = std::vector<EnvironmentVariable>; // preserves order, allows duplicates if you want to mirror POSIX envp.
...
inline IO capture() { return CaptureTarget{}; }
inline IO null_io() { return NullTarget{}; }
inline IO inherit() { return InheritTarget{}; }
...
/// Word with expansion context (for proper quote handling and variable expansion)
struct Word {
    std::string value;
    ...
    explicit Word(std::string val, bool quoted = false, bool expand = true)
...
struct JobWithState {
    ...
    // Helper methods
    [[nodiscard]] bool has_valid_process_group() const noexcept {
        return process_group.is_valid();
    }
};
...
using Statement = std::variant<
    Job,              // Simple command or pipeline
